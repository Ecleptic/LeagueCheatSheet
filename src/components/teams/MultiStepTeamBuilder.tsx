...
export default function MultiStepTeamBuilder({
  className = "",
}: MultiStepTeamBuilderProps) {
  const { gameState, dispatch } = useTeam();
  const [builderState, setBuilderState] = useState<TeamBuilderState>(
    createDefaultTeamBuilderState()
  );
  ...
  const isStepComplete = useCallback(
    (step: TeamBuilderStep): boolean => {
      switch (step) {
        case "team-names":
          return blueTeamName.trim() !== "" && redTeamName.trim() !== "";
        case "player-setup":
          return (
            gameState.blueTeam.players.every(
              (p) => p.name.trim() !== "" && p.position
            ) &&
            gameState.redTeam.players.every(
              (p) => p.name.trim() !== "" && p.position
            )
          );
        case "champion-selection":
          return (
            gameState.blueTeam.players.every((p) => p.champion) &&
            gameState.redTeam.players.every((p) => p.champion)
          );
        case "summoner-spells":
          return (
            gameState.blueTeam.players.every(
              (p) => p.summonerSpells[0] && p.summonerSpells[1]
            ) &&
            gameState.redTeam.players.every(
              (p) => p.summonerSpells[0] && p.summonerSpells[1]
            )
  ...
  const handlePositionChange = (
    ...
    const player =
      gameState[team === "blue" ? "blueTeam" : "redTeam"].players[playerIndex];
    if (player) {
      dispatch({
  ...
  const handleChampionTrade = (
    ...
    const draggedPlayer = teamData.players.find(
  ...
  const handleOpenChampionSelector = (
    team: "blue" | "red",
    playerId: string,
    playerName: string
  ) => {
  ...
  const goToNextStep = () => {
    const stepOrder: TeamBuilderStep[] = [
      "team-names",
      "player-setup",
      "champion-selection",
      "summoner-spells",
    ...
    <div className={`space-y-8 ${className}`}>
      ...
      {builderState.currentStep === "team-names" && (
        <div className="space-y-4">
          ...
          {isStepComplete("team-names") && (
            <div className="text-center text-green-400 font-medium"></div>
          )}
        </div>
      )}
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-3">
                {gameState.blueTeam.players.map((player, index) => (
                  <div
                    key={player.id}
                    className="flex items-center gap-2 sm:gap-3 p-3 bg-blue-900/30 border border-blue-500/30 rounded-lg"
                  >
                    <select
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-3">
                {gameState.redTeam.players.map((player, index) => (
                  <div
                    key={player.id}
                    className="flex items-center gap-2 sm:gap-3 p-3 bg-red-900/30 border border-red-500/30 rounded-lg"
                  >
                    <select
                      value={player.position || ""}
                      onChange={(e) =>
                    ...
                    <input
                      ...
                      value={player.name || ""}
                      onChange={(e) =>
                        handlePlayerNameChange("red", player.id, e.target.value)
                      }
      ...
      {builderState.currentStep === "champion-selection" && (
        <div className="space-y-6">
          ...
          <p className="text-center text-gray-400 mb-4">
            Select champions for each player. You can drag champions between
            players on the same team to trade.
          </p>
          ...
          {isStepComplete("champion-selection") && (
            <div className="text-center text-green-600 font-medium"></div>
          )}
        </div>
      )}
      ...
      {builderState.currentStep === "summoner-spells" && (
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div className="space-y-4">
              <h3 className="text-lg font-semibold text-blue-400">
                {blueTeamName} - Summoner Spells
              </h3>
              <div className="space-y-3">
                {gameState.blueTeam.players.map((player) => (
                  <div
                    key={player.id}
                    className="flex items-center space-x-3 p-4 bg-blue-900/30 rounded-lg border border-blue-500/30"
                  >
                    <div className="w-6 h-6">
                      {player.position && (
                        <PositionIcon position={player.position} size={24} />
                    ...
                    <div className="flex-1">
                      ...
                      <div className="flex space-x-2">
                        ...
                        <div
                          ...
                          onClick={() =>
                            handleOpenSummonerSpellSelector(
                              "blue",
                              player.id,
                              player.name,
                              0
            ...
            <div className="space-y-4">
              <h3 className="text-lg font-semibold text-red-400">
                {redTeamName} - Summoner Spells
              </h3>
              <div className="space-y-3">
                {gameState.redTeam.players.map((player) => (
                  <div
                    key={player.id}
                    className="flex items-center space-x-3 p-4 bg-red-900/30 rounded-lg border border-red-500/30"
                  >
                    <div className="w-6 h-6">
                      {player.position && (
                        <PositionIcon position={player.position} size={24} />
      ...
      <div className="mt-8 pt-4 border-t border-gray-700">
        ...
        <p className="text-center text-sm text-gray-500 mt-2">
          ...
          ).indexOf(builderState.currentStep) + 1}{" "}
          of 5
        </p>
      </div>
      ...
      {championModal.isOpen && (
        <ChampionSelector
          ...
          onClose={() =>
            setChampionModal((prev) => ({ ...prev, isOpen: false }))
          }
      ...
      {summonerSpellModal.isOpen && (
        <SummonerSpellSelector
          ...
          onClose={() =>
            setSummonerSpellModal((prev) => ({ ...prev, isOpen: false }))
          }
